...
if __name__ == '__main__':
    ...
    if not os.path.exists(qtable_path):
        rospy.loginfo("generating new q-table.dat")
        # set default
    ...
    #rospy.loginfo("table after read")
    #rospy.loginfo(debug_q_table(q_table))
    ...
    def get_best_action(state):
        ...
        #rospy.loginfo(" ".join([str(v) for v in actions_q_values]) + " : " + state)
    ...
    def correct_distance(distances):
        def close():
            ...
            #thresh = ideal_wall_dist * (1 - wall_dist_tol) * .5
            thresh = .215

            return any(map(lambda x: x < thresh, distances))
        def far():
            # if the robot is not close enough on any side (lost wall)
            thresh = ideal_wall_dist * (1 + wall_dist_tol) * 1.5
            return all(map(lambda x: x > thresh, distances[1:]))
    ...
    while not rospy.is_shutdown():
        ...
        if last_state_changed:
            ...
            rospy.loginfo("Chose action {}".format(debug_action_names[possible_actions.index(action)]))
